...
import { Package, MapPin, Tag, Bell, Scale, Settings, Store, ShoppingBag, LogOut, User, RotateCcw } from "lucide-react";
...
export default function Navbar({ user, hasSellerProfile = false, sellerOnboardingCompleted = false }: { user: JwtPayload | null; hasSellerProfile?: boolean; sellerOnboardingCompleted?: boolean }) {
  ...
  const handleVoiceResult = useCallback((text: string) => {
    ...
    setTimeout(() => router.push(`/products/${encodeURIComponent(text)}`), 600);
  }, [router]);
  ...
  function handleSearch() {
    ...
    if (searchCategory !== "All Categories") {
      router.push(`/products/${searchCategory.toLowerCase()}?${params.toString()}`);
    } else {
  ...
  return (
    <>
      ...
      {pathname.startsWith("/vendor") ? (
        /* ── Seller Navbar (preserved as-is visually with new colors) ────── */
        <nav className="bg-[#1a211e] px-3 md:px-5 py-0 h-14 flex items-center gap-2 md:gap-4">
          <button aria-label="Open Menu" aria-expanded={mobileMenuOpen} className="md:hidden bg-transparent border-none text-white cursor-pointer p-1" onClick={() => setMobileMenuOpen(true)}>
            <MenuIcon />
          </button>

          <div onClick={() => router.push("/vendor/dashboard")} className="cursor-pointer flex items-center gap-2 shrink-0">
            <div className="w-7 h-7 bg-white/20 rounded flex items-center justify-center text-white text-[10px] font-bold">B2</div>
            <span className="text-white font-bold text-sm hidden md:inline">{user?.name || "Seller Hub"}</span>
          </div>

          <div className="hidden md:flex items-center gap-1 ml-4 overflow-x-auto flex-1">
            {[
              ...
              <button
                ...
                className={`bg-transparent border-none h-10 px-3 text-[13px] font-medium cursor-pointer whitespace-nowrap rounded transition ${pathname === link.href || pathname.startsWith(link.href + "/")
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button
              className="p-2 hover:bg-white/10 rounded text-white/80 hover:text-white"
              title="Notifications"
            >
              ...
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[#cc2e39] rounded-full" />
            </button>
            ...
            <div ref={dropdownRef} className="relative">
              <button
                ...
                aria-haspopup="true"
                className="flex items-center gap-2 px-2 py-1.5 text-white/90 hover:bg-white/10 rounded"
              >
              ...
              {dropdownOpen && (
                <div className="absolute top-full right-0 mt-1 min-w-[200px] bg-white rounded-lg border border-[#e0e0e0] overflow-hidden z-[300]">
                  <button onClick={() => { router.push("/vendor/profile"); setDropdownOpen(false); }} className="w-full bg-transparent border-none px-4 py-2.5 text-[13px] text-[#1a211e] cursor-pointer flex items-center gap-2.5 text-left hover:bg-[#eef1f0]"> <User className="w-4 h-4 shrink-0" /> My Seller Profile</button>
                  <button onClick={() => { router.push("/settings"); setDropdownOpen(false); }} className="w-full bg-transparent border-none px-4 py-2.5 text-[13px] text-[#1a211e] cursor-pointer flex items-center gap-2.5 text-left hover:bg-[#eef1f0]"> <Settings className="w-4 h-4 shrink-0" /> Account Settings</button>
                  <button onClick={() => { router.push("/"); setDropdownOpen(false); }} className="w-full bg-transparent border-none px-4 py-2.5 text-[13px] text-[#1a211e] cursor-pointer flex items-center gap-2.5 text-left hover:bg-[#eef1f0]"> <ShoppingBag className="w-4 h-4 shrink-0" /> Switch to Buyer Mode</button>
                  <button onClick={handleLogout} className="w-full bg-transparent border-none px-4 py-2.5 text-[13px] text-[#cc2e39] font-medium cursor-pointer flex items-center gap-2.5 text-left hover:bg-[#eef1f0]"> <LogOut className="w-4 h-4 shrink-0" /> Logout</button>
                </div>
              )}
            </div>
          </div>
          ...
          <nav className="bg-white border-b border-[#e0e0e0]">
            <div className="max-w-[1440px] mx-auto px-3 md:px-8 h-16 flex items-center gap-2 md:gap-4">
              ...
              <div onClick={() => router.push("/")} className="cursor-pointer shrink-0 mr-1 md:mr-2">
                <div className="text-[#1a211e] font-extrabold text-[20px] md:text-[22px] tracking-tight leading-none" style={{ letterSpacing: '-0.03em' }}>B2World</div>
              </div>
              ...
              <div className="hidden xl:flex items-center gap-0.5">
                {NAV_CATEGORIES.map(cat => (
                  <button
                    ...
                    className="bg-transparent border-none text-[13px] font-bold text-[#363537] cursor-pointer py-1 px-2.5 uppercase tracking-[0.057em] transition-colors duration-200 hover:text-[#1a211e]"
                  >
                    {cat}
                  </button>
                ))}
