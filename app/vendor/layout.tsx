...
import {
  LayoutDashboard, ShoppingBag, Package, BarChart3, Percent,
  Star, Warehouse, MessageSquare, RotateCcw, Bell, User, Settings,
  LogOut, ChevronDown, Menu, MapPin, Ticket,
} from "lucide-react";
...
const NAV_LINKS = [
  ...
  { label: "Pincodes",   href: "/vendor/pincodes",  icon: MapPin },
  { label: "Coupons",    href: "/vendor/coupons",   icon: Ticket },
  { label: "Payouts",    href: "/vendor/payouts",   icon: Percent },
  { label: "Promotions", href: "/vendor/promotions", icon: Star },
  ...
  useEffect(() => {
    fetch("/api/seller/me")
      ...
      .then(d => {
        ...
        if (d?.storeName) setSellerName(d.storeName);
        else if (d?.name) setSellerName(d.name);
        setLoading(false);
      })
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <nav className="sticky top-0 z-40 bg-b2w-navy text-white h-14 flex items-center gap-2 md:gap-3 px-3 md:px-5 shadow-sm shrink-0">
        ...
        <div onClick={() => router.push("/vendor/dashboard")} className="cursor-pointer flex items-center gap-2 shrink-0">
          ...
          <span className="text-white font-bold text-sm hidden md:inline">{sellerName}</span>
        </div>
          ...
          <div ref={profileRef} className="relative">
            ...
            {profileOpen && (
              <div className="absolute top-full right-0 mt-1 min-w-[200px] bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden z-50">
                <button
                  onClick={() => { router.push("/vendor/profile"); setProfileOpen(false); }}
                ...
                >
                  <User className="w-4 h-4 shrink-0" /> My Seller Profile
                </button>
                <button
                ...
                <button
                  onClick={() => { window.location.href = "http://localhost:3000"; setProfileOpen(false); }}
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-[13px] text-gray-700 cursor-pointer text-left hover:bg-gray-50 border-b border-gray-100 bg-transparent border-none"
                >
                  <ShoppingBag className="w-4 h-4 shrink-0" /> Switch to Buyer Mode
                </button>
                <button
